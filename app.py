...
def _quota_config() -> list[dict[str, Any]]:
    """Return a normalized quota view for current state and legacy installs.

    3.1.2 could leave migrated users without ``quota_gib`` and could trust a
    stale/malformed quotas.json.  Runtime accounting now merges only valid
    persisted entries with the authoritative local user/settings state.
    """
    state = home_state()
    settings = state.get("settings", {}) if isinstance(state.get("settings"), dict) else {}
    users = state.get("users", {}) if isinstance(state.get("users"), dict) else {}
    default_gib = max(1, int(settings.get("default_quota_gib", 64) or 64))
    shared_per_user_gib = max(1, int(settings.get("shared_contribution_gib", 15) or 15))

    normalized: dict[str, int] = {}
    raw = _load_json_file(PATHS.quotas_file, {"quotas": []}).get("quotas", [])
    if isinstance(raw, list):
        for item in raw:
            ...
            if username and match and int(match.group(1)) > 0:
                normalized[username] = int(match.group(1))
    ...
    for username, profile in users.items():
        ...
        if profile.get("enabled", True):
            active_users += 1
        try:
            quota_gib = int(profile.get("quota_gib", 0) or 0)
        except (TypeError, ValueError):
            quota_gib = 0
        normalized[str(username)] = max(1, quota_gib or default_gib)

    if active_users:
        normalized["shared"] = max(normalized.get("shared", 0), active_users * shared_per_user_gib)

    return [{"username": username, "hard": f"{gib}G"} for username, gib in sorted(normalized.items()) if gib > 0]
