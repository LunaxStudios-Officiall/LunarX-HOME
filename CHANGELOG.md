...
## 3.1.3 — Android/PRoot field reliability

- Fixes browser login form serialization by naming username/password controls and reports invalid credentials correctly.
- Makes migrated logical quotas visible and enforceable by normalizing legacy user quota state and ignoring malformed legacy quota rows.
- Adds `tigervnc-tools`, supports `tigervncpasswd`, and starts XFCE through `dbus-launch` when available.
- Replaces misleading systemd-unit `unknown` rows in Android/PRoot with capability-aware portable service status.
- Makes PRoot backups buffered and filtered so Git pack files cannot trigger CPython `sendfile` `EINVAL`.
- Streams APT progress instead of appearing frozen during desktop dependency installation.
- Adds a Termux-native one-command updater that installs/updates 3.1.3, configures persistent PRoot supervision, preserves SSH boot, and performs a final health check.
