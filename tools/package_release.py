...
def write_zip(root: Path, target: Path, files: list[Path], generated: dict[str, bytes]) -> None:
    with zipfile.ZipFile(target, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=9) as archive:
        for path in files:
            ...
            info = zipfile.ZipInfo(relative, STAMP); info.external_attr = (0o755 if path.name in {"install.sh", "bootstrap.sh", "lunarx-admin.py", "lunarx-terminal", "lunarxctl", "lunarx_installer.py", "lunarx-desktop-session.py", "lunarx-desktop-governor.py", "lunarx-user-apps", "termux-update.sh"} else 0o644) << 16; info.compress_type = zipfile.ZIP_DEFLATED; archive.writestr(info, path.read_bytes())
...
def main() -> int:
    ...
    report = (root / "FINAL_BUILD_REPORT.md").read_bytes() if (root / "FINAL_BUILD_REPORT.md").is_file() else b"# LunarX Home 3.1.3 final build report\n\nSee VALIDATION.json and SHA256SUMS.txt for evidence.\n"
