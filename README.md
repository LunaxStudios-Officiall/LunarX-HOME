...
LunarX Home 3.1.3 is the Android/PRoot field-reliability release built directly on the validated 3.1.2 foundation. It preserves the approved desktop sidebar/mobile drawer, catalog and data model while fixing the real phone issues discovered during Moto G82 deployment.

## What changed in 3.1.3

3.1.3 is the phone-field reliability release. It fixes the browser login form, legacy quota migration/reporting, Android/PRoot service status, TigerVNC tooling/session startup, PRoot-safe updater backups, visible package-install progress, and persistent Termux supervision.

### Android / Termux one-command update

```sh
curl -fsSL https://raw.githubusercontent.com/LunaxStudios-Officiall/LunarX-HOME/3.1.3/termux-update.sh | sh
```

Run that command from the Termux `~ $` prompt, not from inside Ubuntu PRoot. Existing `/opt/lunarx/runtime` data/configuration and user accounts are preserved.
...
Run the installer from the newly extracted **3.1.3** release:
...
The 3.1.3 source gate passed **67/67 Python tests** plus Python/JavaScript/shell/JSON/catalog validation and **406/406 Chromium checks** across the 11 requested phone/tablet/desktop viewports. The earlier 3.1.2 rollback and compatibility evidence remains preserved as historical regression evidence; 3.1.3 adds dedicated field-fix tests for the phone-discovered failures.
