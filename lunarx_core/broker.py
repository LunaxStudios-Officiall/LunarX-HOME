...
def _portable_desktop_session(username: str, paths: PathConfig, info: PlatformInfo) -> dict[str, Any]:
    ...
    vncpasswd = shutil.which("tigervncpasswd") or shutil.which("vncpasswd")
...
def _download_https(url: str, *, max_bytes: int) -> bytes:
    ...
    try:
        request = urllib.request.Request(url, headers={"User-Agent": "LunarX-Home/3.1.3"})
...
def _service_status(info: PlatformInfo, units: list[object] | None = None) -> dict[str, Any]:
    if info.service_manager == "lunarxctl":
        desktop_state = "active" if info.desktop_available else "unavailable"
        tailscale_state = "host-owned" if info.is_proot or info.is_termux else ("active" if info.tailscale_installed else "unavailable")
        return {
            "service_manager": "lunarxctl",
            "supported": True,
            "units": [
                {"unit": "lunarx-home", "state": "active", "message": "Managed by the LunarX portable supervisor"},
                {"unit": f"desktop:{info.desktop_backend}", "state": desktop_state, "message": "Capability-detected desktop provider"},
                {"unit": f"storage:{info.quota_mode}", "state": "active", "message": "Persistent directory storage provider"},
                {"unit": "tailscale", "state": tailscale_state, "message": "Android host owns Tailscale in PRoot mode" if tailscale_state == "host-owned" else "Runtime-detected VPN provider"},
            ],
        }
    names = [str(item) for item in (units or []) if str(item) and len(str(item)) < 160]
    return {"service_manager": info.service_manager, "supported": info.service_manager != "portable", "units": [{"unit": name, "state": "unknown", "message": "Inspect this service using the host service manager"} for name in names]}
...
def handle(payload: dict[str, Any], *, paths: PathConfig | None = None, info: PlatformInfo | None = None) -> tuple[bool, dict[str, Any]]:
    ...
    try:
        ...
        if action == "list_users":
            ...
            for name, item in sorted(state["users"].items()):
                if not isinstance(item, dict):
                    continue
                public = _user_public(name, item)
                public["used_bytes"] = _directory_size(config.data_root / "users" / name)
                users.append(public)
        ...
        if action == "server_app":
            ...
            state.setdefault("server_apps", {})[app_id] = {"installed": operation != "uninstall", "version": "3.1.3", "updated_at": int(time.time())}
