...
DESKTOP_PACKAGES = ["xfce4", "xfce4-goodies", "tigervnc-standalone-server", "novnc", "websockify", "xrdp"]
PROOT_PACKAGES = ["ca-certificates", "curl", "file", "ffmpeg", "gnupg", "python3", "python3-cryptography", "python3-pil", "unzip"]
PROOT_DESKTOP_PACKAGES = ["dbus-x11", "xfce4", "xfce4-goodies", "tigervnc-standalone-server", "novnc", "websockify"]
...
def run(command: list[str], *, check: bool = True, input_text: str | None = None, timeout: int = 120) -> subprocess.CompletedProcess[str]:
    try:
        result = subprocess.run(command, check=False, text=True, input=input_text, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=timeout, env={"PATH": "/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin", "LANG": "C.UTF-8"})
...
class Installer:
    ...
    def ensure_config(self, admin: str | None) -> str | None:
        ...
        state = _json_load(state_path, {"schema_version": VERSION, "settings": {"shared_contribution_gib": 15, "default_quota_gib": 64, "desktop_idle_timeout_minutes": 10, "theme_mode": "dark", "server_name": "LunarX Home"}, "users": {}, "server_apps": {}, "installed_apps": {}, "storage": {"provider": "logical", "members": []}})
        state["schema_version"] = VERSION
        state.setdefault("users", {})
        if admin and admin not in state["users"]:
            state["users"][admin] = {"display_name": admin.title(), "avatar": "", "theme": "dark", "enabled": True, "is_admin": True, "role": "admin", "auth_provider": "pam", "permissions": {"desktop_access": True, "desktop_app_install": True, "shared_access": True, "drive": True, "photos": True, "files": True}}
        ...
        if not quotas.exists():
            atomic_text(quotas, json.dumps({"schema_version": VERSION, "provider": "logical", "quotas": []}, indent=2) + "\n")
